...
namespace MTE
{
    internal class ObjectPainter : IEditor
    {
        ...
        public void DoArgsGUI()
        {
            ...
            // detail list box
            SelectedIndex = OnObjectDetailSelectorGUI(SelectedIndex);
        ...
        private int OnObjectDetailSelectorGUI(int selectedIndex)
        {
            if (this.detailList == null || this.detailList.Count == 0)
            {
                EditorGUILayout.HelpBox(StringTable.Get(C.Warning_NoObjectDetail), MessageType.Warning);
                return selectedIndex;
            }

            scrollPos = EditorGUILayout.BeginScrollView(scrollPos, "box", GUILayout.ExpandHeight(true));
            for (var i = 0; i < this.detailList.Count; i += 4)
            {
                EditorGUILayout.BeginHorizontal();
                {
                    for (var j = 0; j < 4; j++)
                    {
                        ...
                        var detailIndex = i + j;
                        ObjectDetailPreview.DrawObjectPreview(this.detailList[detailIndex], rect);
        ...
        {
            var objectLoader = Object.FindObjectOfType<ObjectLoader>();
            if (objectLoader == null)
            {
                return;
            }
            var detailListAsset = objectLoader.detailList;
            if (detailListAsset != null && detailListAsset.list != null)
            {
                detailList = detailListAsset.list;
                MTEDebug.Log($"ObjectDetailList loaded from ObjectLoader on GameObject {objectLoader.gameObject.name}");
            }
            else
            {
                MTEDebug.LogError("Cannot find Object Detail list asset. Please create an Object Detail asset and assign it to the ObjectLoader.");
            }
        }
